import { ArrowRight, Lock, ShieldCheck, Mail, User as UserIcon, ArrowLeft } from 'lucide-react';
...
const Register: React.FC = () => {
  ...
  const { registerWithEmail, signInWithGoogle, currentUser, userProfile, isAdmin } = useAuth();
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center p-4 md:p-6 relative overflow-hidden font-sans">
      <SEO title="Register | Growplex" description="Create a new Growplex account." noindex={true} />
      {/* Back to Home Button */}
      <Link 
        to="/" 
      ...
      >
        ...
        <div className="text-center mb-10">
          <Link to="/">
            ...
            <h1 className="text-4xl md:text-5xl font-display font-black text-white mb-3 tracking-tight italic hover:text-[#E8B84B] transition-colors cursor-pointer">
              Join <span className="text-[#E8B84B]">GROWPLEX</span>
            </h1>
          </Link>
          <p className="text-slate-400 font-medium">Create your account to elevate your hustle.</p>
        </div>
        ...
        <div className="glass-card p-8 md:p-10 border border-white/5 rounded-[32px] bg-white/[0.03] backdrop-blur-2xl shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)]">
          <form onSubmit={handleRegister} className="space-y-6">
            ...
            <div>
              <label className="block text-[11px] font-bold uppercase tracking-[0.2em] text-[#E8B84B] mb-3 ml-1">Confirm Password</label>
              <div className="relative group">
                <Lock className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-500" size={20} />
                <input
                  type="password"
          ...
          <p className="mt-8 text-center text-slate-400 text-sm font-medium px-4">
            Already have an account? <Link to="/login" className="text-[#E8B84B] hover:underline font-bold">Log in</Link>
          </p>
        </div>
      </motion.div>
    </div>
